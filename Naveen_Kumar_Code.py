# -*- coding: utf-8 -*-
"""AIB_ModelCreation.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1IFml0umtwHFTWxzh26TIehWQ3NRXcbNd
"""
...
df = pd.read_csv("/content/drive/MyDrive/CA_2/AIB_Labelled.csv")
...
print(df.info())
tfidf = TfidfVectorizer(min_df=.0001, ngram_range=(1,3)) # min_df=.00086 means that each ngram (unigram, bigram, & trigram) must be present in at least 20 documents for it to be considered as a token (23305*.00086=20). This is a clever way of feature engineering
tfidf.fit(data) # learn vocabulary of entire data
data_tfidf = tfidf.transform(data) # creating tfidf values
pd.DataFrame(pd.Series(tfidf.get_feature_names_out())).to_csv('/content/drive/MyDrive/CA_2/AIB_Vocab.csv', header=False, index=False)
...
joblib.dump(clf, '/content/drive/MyDrive/CA_2/AIB_Reviews.sav')

#Deployment of the Model
model = joblib.load('/content/drive/MyDrive/CA_2/AIB_Reviews.sav')
vocabulary = pd.read_csv('/content/drive/MyDrive/CA_2/AIB_Vocab.csv', header=None)
vocabulary_dict = {}
...
df = pd.read_csv("/content/drive/MyDrive/CA_2/AIB_Unlabelled.csv")
pd.set_option('display.max_colwidth', None) # Setting this so we can see the full content of cells
pd.set_option('display.max_columns', None) # to make sure we can see all the columns in output window
...
df.to_csv('/content/drive/MyDrive/CA_2/AIB_Predicted_ratings.csv', index=False)
...
u = umap.UMAP(n_components=2, n_neighbors=5, min_dist=0.0)
